 ...
 *
 * Run this class once after the schema is created but before logging into the UI.
 */
public class BootstrapDataLoader {
	...
	private static final String ADMIN_LOGIN = "CTO@samskriti.org";
	private static final String ADMIN_PASSWORD = "CTO123$%^";
	private static final String ADMIN_EMAIL = "CTO@samskriti.org";

	private static final List<MenuSeed> MENU_SEEDS = 
        /*Id, Defaultstatus, LeftPanelLink, MenuLevel, MenuLink, MenuName, MenuOrder, ParentId, RequestId, ShortKey, StatusMsg */
        Arrays.asList(
                new MenuSeed(1L, "1", "Root", 0, "Root", "ROOT", 0, null, "0", null, "WASP Configuration"),
                new MenuSeed(101L, "1", "widget.leftprofile|widget.mybookmarks", 0, "/homePageAction.action", "Dashboard", 1, 1L, "1",
                                null, "Home"),
                ...
                new MenuSeed(10302L, "0", null, 1, "/searchPageAction.action", "Search", 1, 103L, "10302", null,
                                "Search Users"),
                new MenuSeed(10303L, "0", null, 1, "/generateResetPasswordIdPageAction.action", "Reset Password", 1, 103L, "10303", null,
                                "Reset password"),
                new MenuSeed(10401L, "0", "widget.leftprofile|widget.mybookmarks", 1, "/logoutaction.action", "Sign out", 1,
                                104L, "10401", null, "Sign out"),
                new MenuSeed(10402L, "0", "widget.leftprofile|widget.mybookmarks", 1, "/changePasswordAction.action", "Change Password", 1,
                ...
                new MenuSeed(10502L, "0", null, 1, "/searchForManuscript.action", "Search", 1, 105L, "10502", null,
                                "Search Manuscripts"),
                new MenuSeed(10601L, "0", null, 1, "/createSearchFormAction.action", "Language", 0, 106L, "10601", null,
                                "Language"),
                new MenuSeed(10602L, "0", null, 1, "/createSearchFormAction.action", "Subject", 0, 106L, "10602", null,
                                "Subject"),
                new MenuSeed(10603L, "0", null, 1, "/createSearchFormAction.action", "Script", 0, 106L, "10603", null,
                                "Script"),
                new MenuSeed(10604L, "0", null, 1, "/createSearchFormAction.action", "Bundle", 0, 106L, "10604", null,""),
                new MenuSeed(10605L, "0", null, 1, "/createSearchFormAction.action", "Tag", 0, 106L, "10605", null,""),
                new MenuSeed(10606L, "0", null, 1, "/createSearchFormAction.action", "Specific Category", 0, 106L, "10606", null,"")
        );

	private static final Set<Long> ADMIN_MENU_IDS = new HashSet<>(
        Arrays.asList(101L, 102L, 10201L, 10202L, 103L, 10301L, 10302L, 10303L, 104L, 10401L, 10402L, 105L, 10501L, 10502L, 106L, 10601L, 10602L, 10603L, 10604L, 10605L, 10606L, 107L));
	...
	private Long seedDefaultLocation(Session session) throws Exception {
		...
		if (locationCount != null && locationCount.longValue() > 0) {
			LocationMasterBean existing = (LocationMasterBean) session.createQuery("from LocationMasterBean order by id")
					.setMaxResults(1)
					.uniqueResult();
	...
	private void seedAdministrator(Session session, Long roleId, Long locationId) throws Exception {
		...
		UserLoginDetailsBean existingUser = (UserLoginDetailsBean) session
				...
				.setParameter("login", ADMIN_LOGIN.toLowerCase())
				.uniqueResult();
		...
		UserLoginDetailsBean adminLogin = new UserLoginDetailsBean();
		adminLogin.setLoginId(ADMIN_LOGIN);
		adminLogin.setPassword(SecurePasswordUtil.hashUserPassword(ADMIN_PASSWORD, ADMIN_LOGIN));
		adminLogin.setRefrenceFkId(employee.getId());
		adminLogin.setStatus(UserStatusEnum.ACTIVE.getValue());
		adminLogin.setType(UserTypeEnum.ADMIN.getValue());
		adminLogin.setIsDeleted(0);
		CustomBeanUtil.setBaseValues(adminLogin, false);
		session.save(adminLogin);

		UserRoleDetailsBean roleDetails = new UserRoleDetailsBean();
		roleDetails.setRoleMasterFkId(roleId);
		roleDetails.setUserLoginDetailsFkId(adminLogin.getId());
		...
		OmdsUserRoleDetails.setUserLoginDetailsFkId(adminLogin.getId());
		session.save(OmdsUserRoleDetails);

		System.out.println("Created administrator account (" + ADMIN_LOGIN + "/" + ADMIN_PASSWORD + "}");
	}
	...
	private static final class MenuSeed {
		...
		private MenuSeed(Long id, String defaultStatus, String leftPanelLink, Integer menuLevel, String menuLink,
				String menuName, Integer menuOrder, Long parentId, String requestId, String shortKey, String statusMsg) {
			this.id = id;
