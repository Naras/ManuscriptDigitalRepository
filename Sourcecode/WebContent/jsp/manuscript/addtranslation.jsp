...
<div class="form" style="margin-left: 190px;">
	<div class="container" style="padding-top: 0px;">
		...
		<s:form action="addUpdateDigitalDocument" id="derivedWorkForm">
			<s:hidden id="manuscriptId" name="parentVO.id" />
			
			<s:hidden id="transmanuscriptId" name="digitalManuscriptVO.id" />
			
			<s:hidden id="transLanguage" name="translatedDocumentVO.language" />
			<s:hidden id="transScript" name="translatedDocumentVO.script" />
			
			<s:hidden name="presentReviewer2" id="isPresentReviewer2" />
			<s:hidden id="scribeId" name="parentVO.scribeVO.id" />
			<s:hidden id="commentatorId"
				name="parentVO.commentatorVO.id" />
			<s:hidden id="translatorId"
				name="parentVO.translatorVO.id" />
			<s:hidden id="organisationId"
				name="parentVO.organisationVO.id" />
			<s:hidden id="organisationType"
				name="parentVO.organisationVO.type" />
			...
			<s:hidden id="manuscriptTextTrans" name="translatedDocumentVO.detailsVO.text" />
			...
			<h3 style="margin: 0px;">
				Online Derived Work - <span style="color: #FFA500;"><s:text
						name="parentVO.name"></s:text></span>
			</h3>
			<div>
			...
			<div class="image-container panel" style="margin-bottom: 5px;">
							...
							<div class="panel-body" oncontextmenu="return false" style="overflow-y: scroll;padding-top: 3px;"></div>
			</div>
			<div id="frame">
				<div class="image-details">
					<div
						...
						<%-- <div class="tag-container panel" style="overflow-y: scroll;">
							...
							<div class="tag-data" style="padding-top: 10px;">
								<s:iterator value="parentVO.tagList" status="var">

									<div class='tag-data-shell'>
										<s:hidden id="%{#var.index}"
											name="parentVO.tagList[%{#var.index}].id"></s:hidden>
											<s:property value="name"/>
										<s:textfield cssStyle="border-width: 0px;width: 100%;"
											name="parentVO.tagList[%{#var.index}].name"></s:textfield>
					...
					<div
						...
						<div class="transcriber-container" style="height:85%;">
							<s:textarea 
								cssClass="form-control transcriber" rows="32" readonly="true"></s:textarea>
								<div class="searchDiv" style="display: none;" id="showdiv" onclick="replace();"></div>
						</div>
						
						<div class="button-container">
						<div  id="replaceEmbedFilepath" class="col-md-4" style="padding: 0px; margin-top: 0px; width: 28%;display: none;">
								</div>
						<div  id="embedFilepath" class="col-md-4" style="padding: 0px; margin-top: 0px; width: 28%;display: none;">
								</div>
						<!-- <label class="col-md-3 control-label" style="padding: 0px;  width: 140px; margin-top: 8px;">Upload Audio/Video</label>
							<div class="col-md-1" style="padding: 0px;">
								<button type="button"
											class="btn btn-lg btn-warning btn-block btn-state-1" style="float: left;" id="">Browse</button>
							</div> -->
							<div class="col-md-3 control-label display-span" style="margin-left: 13%">
							    <span>Frame No:</span> 
						...
						<%-- <div class="transcriber-container">
							<s:textarea 
								cssClass="form-control transcriber1" id="translatedData" rows="11"></s:textarea>
						</div> --%>
					</div>
					<div class="right-column ui-widget-content ui-resizable" style="border: none;width: 44.25%;">
						...
						<div class="panel panel-success">
							<div class="panel-heading" style="height: 25px">Tranlation<span class="glyphicon glyphicon-comment glyphiconmargin" style="margin-top: 8px;"></span></div>
								<s:textarea 
									cssClass="form-control transcriber1" id="translatedData" rows="16"></s:textarea>
							</div>
						</div>
							...
							<div class="panel-group" id="accordion1">
									<div id="accordionPanelHeader" class="panel panel-default">
											...
											<div id="collapseOne1" class="panel-collapse collapse in">
												...
												style="overflow-y: scroll; display: block; margin-top: 1%; height: 61%;background-color: #fff;">
											</div>
									...
									<div id="accordionPanelHeader" class="panel panel-default">
										...
										<div id="collapseTwo1" class="panel-collapse collapse">
											<div class="panel-body"
												style="background:#ffffff; overflow-y: scroll; display: block; margin-top: 1%; height: 72%;font-family: trebuchet ms ; font-size: 11.5px;font-style: italic;">
												<s:textarea readonly="true" cssStyle="width:100%;height:100%;font-size: 13px;font-style: italic;" name="parentVO.summary" />
											</div>
										</div>
									</div>
									<div id="accordionPanelHeader2" class="panel panel-default">
										...
										<div id="collapseThree1" class="panel-collapse collapse">
											<div class="panel-body" style="background:#ffffff; margin-top: 1%; margin-right: 1%; height: 72%;">
												<div>
													<label class="control-label">Author:</label>
													<s:property value="parentVO.authorVO.name" />
												</div>
												<div>
													<label class="control-label">Source:</label>
													<s:property value="parentVO.organisationVO.name" />
												</div>
												<div>
													<label class="control-label">Manuscript Id:</label>
													<s:property value="parentVO.manuscriptId" />
												</div>
												<div>
													<label class="control-label">Material:</label>
													<s:property value="parentVO.materialVO.name" />
												</div>
												<div>
													<label class="control-label">Type Of Work:</label>
													<s:property value="parentVO.typeOfWork" />
												</div>											
											</div>
										</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		/* var seleLang = document.getElementById("transLanguage").value;
		var selScript = document.getElementById("transScript").value;
		
		var element = document.getElementById('imeLanguage');
	    element.value = seleLang;
	    
	    var element2 = document.getElementById('imeSelector');
	    element2.value = selScript; */
	});
	...
	function initializeIme() {
		...
		imeselector = $( '.transcriber1' ).data( 'imeselector' );
	...
	function imageChange(imgno,lastimgno){
		...
		$('#replaceEmbedFilepath').css('display','block');
		...
		$('#translatedData').val(fileDiskPathObject[imgno].translatedtext);
		...
		if($('#audioPath').val()!= null && $('#audioPath').val() !=""){
			getAudioFile();
		}else{
			 var inerVar = '<embed src="" autostart="false" controller="true" width="100%" height="55">'+
            '</embed>';
	...
	/*  $('#pdf').click(function(){ */
		 if(($('#manuscriptLabel').prop('checked'))){
			 ...
			 document.getElementById(rtf).href=(document.getElementById(rtf).href+  
					  '&id='+frmId+'&startPage=0&endPage=0&recordType=1');
		 }else{
		  var frmId = $('#frameId').val();
		  document.getElementById(rtf).href=(document.getElementById(rtf).href+  
				  '&id='+frmId+'&startPage=0&endPage=0&recordType=1');  
...
function generateSelectedReport(){
	...
	for(var i = 0; i<radios.length; i++){
		...
		window.location = '${pageContext.request.contextPath}/exportReport.action?requestId='+<%=requestd%>+ '&jasperPath=/report/OnlyTextRepot.jasper&id='+manuscriptId +'&type='+docType+'&startPage=0&endPage=0&recordType=1';
	}else if(radioId == 2){
		window.location = '${pageContext.request.contextPath}/exportReport.action?requestId='+<%=requestd%>+ '&jasperPath=/report/FullManuscriptWithFrame.jasper&id='+manuscriptId +'&type='+docType+'&startPage=0&endPage=0&recordType=1';
	}else if(radioId == 3){
		window.location = '${pageContext.request.contextPath}/exportReport.action?requestId='+<%=requestd%>+ '&jasperPath=/report/ManuscriptReport.jasper&id='+manuscriptId +'&type='+docType+'&startPage=0&endPage=0&recordType=1';
	}else if(radioId == 4){
		...
		window.location = '${pageContext.request.contextPath}/exportReport.action?requestId='+<%=requestd%>+ '&jasperPath=/report/FullManuscriptWithFrame.jasper&id='+manuscriptId +'&type='+docType+'&recordType=1&startPage='+stPage+'&endPage='+endPage;
 ...
 $('#manuscriptLabel').click(function(){
	...
	$('#translatedData').val($('#manuscriptTextTrans').val());
 ...
 function owdSave(){
	 ...
	 if(!($('#manuscriptLabel').prop('checked'))){
		 ...
		 type=1;
		 id=$('#transmanuscriptId').val();
	 }
	 $.ajax({
		    type: "POST",
		    url: 'framePartialSaveTrans.action?requestId='+<%=requestd%>+'&frameId='+ id +'&manuscriptId='+ $('#transmanuscriptId').val()+'&type='+ type+'&language='+ $('#transLanguage').val(),
		   	dataType: 'json',
		    data: { comment:text },
		   	cache: false,
		    success: function(data) {
		    	...
		    	for(var i = 0; i < Object.keys(fileDiskPathObject).length; i++) {
					...
					if(fileDiskPathObject[i].id == $('#frameId').val()) {
						fileDiskPathObject[i].translatedtext = text;
 ...
 function savecomment() {
	...
	if(commentTxt.length > 0) {
		$.ajax({
			type :"GET",
			dataType:"json",
			url:'savecommentforframe.action?requestId='+<%=requestd%>+'&text='+ $('#commentBox').val()+'&frameId='+ $('#frameId').val()+'&manuscriptId='+ $('#transmanuscriptId').val()+'&currentime='+$('#commentDate').val(),
			cache : false,
			success:function(data) {
 ...
 function getAudioFile() {
	 ...
	 if(!($('#manuscriptLabel').prop('checked'))){
			...
			audioPath=$('#manuscriptAttachment').val();
		}
	 $.ajax({
		    type: "GET",
		    url: 'getAudioFile.action?requestId='+<%=requestd%>+ '&audioPath='+ audioPath,
		    dataType: 'json',
		    cache: false,
		    success: function(data) {
		    	if(data.audioPath!=null){
					...
					$('#audioEdit').css('display','block');
					 ...
					 $('#replaceEmbedFilepath').css('display','none');
					 $('#embedFilepath').css('display','block');
